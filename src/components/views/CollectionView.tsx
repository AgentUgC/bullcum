      ...
      <div style={{ padding: 20, maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{ padding: '8px 14px', background: configMode ? 'var(--peach)' : 'var(--cream)', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 13 }}
            >
              {configMode ? '退出配置' : '批量配置'}
            </button>
            <button
              onClick={() => { setTourActive(true); showToast('开始全景车间巡回', 'info'); setTimeout(() => setTourActive(false), 5000); }}
              className="pixel-border"
              style={{ padding: '8px 14px', background: 'var(--cream)', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 13 }}
            >
              全景巡回
            </button>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 10 }}>
          {state.pumps.map(pump => {
            ...
            return (
              <div
                key={`${pump.coreType}-${pump.pumpIndex}`}
                className="pixel-border-thin"
                style={{
                  padding: 10,
                  background: pump.status === 'running' ? 'rgba(255,181,167,0.15)' : pump.status === 'done' ? 'rgba(109,160,109,0.15)' : 'var(--cream)',
                  borderLeft: `3px solid ${pump.coreType === 'ultimate' ? 'var(--q-superior)' : pump.coreType === 'superior' ? 'var(--q-superior)' : pump.coreType === 'premium' ? 'var(--q-premium)' : 'var(--q-standard)'}`,
                  cursor: 'pointer',
                  transition: 'transform 0.15s',
                  position: 'relative',
                }}
                onClick={() => {
                  if (bull) setInspectBullId(bull.id);
                }}
              >
                ...
                {bull ? (
                  <>
                    ...
                    <div style={{
                      width: '100%', height: 3, background: 'var(--light-wood)', marginTop: 4,
                    }}
                    >
                      <div style={{
                        width: `${(bull.dailyCollected / bull.dailyMax) * 100}%`,
                        height: '100%',
                ...
                <div style={{
                  ...
                  background: pump.status === 'running' ? 'var(--rose-gold)' : pump.status === 'done' ? 'var(--success)' : 'var(--light-wood)',
                  animation: pump.status === 'running' ? 'breathe 1.5s ease-in-out infinite' : 'none',
                }} />
              </div>
            );
          })}
        ...
        {/* Batch Configurator Overlay */}
        {configMode && (
          <div style={{
            position: 'fixed', inset: 0, background: 'rgba(255,248,240,0.92)', zIndex: 200,
                  ...
                  <div key={core.type} style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      {Array.from({ length: core.level }).map((_, i) => {
                        ...
                        const bull = pump?.bullId ? state.bulls.find(b => b.id === pump.bullId) : null;
                        return (
                          <div key={i} className="pixel-border-thin" style={{
                            width: 80, padding: '6px 8px', background: bull ? 'rgba(255,181,167,0.1)' : 'var(--warm-milk)',
        ...
        {inspectBullId && (
          <div style={{
            ...
            background: 'rgba(250,243,232,0.97)', zIndex: 150,
            borderLeft: '2px solid var(--espresso)', boxShadow: '-4px 0 16px rgba(0,0,0,0.1)',
                ...
                <>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {['鼓励', '调整姿势', '质检确认', '亲密触摸'].map(action => (
                      <button
                        key={action}
                        onClick={() => showToast(`对 ${bull.name} 执行: ${action}`, 'success')}
                        className="pixel-border"
                        style={{ padding: '6px 12px', background: 'var(--peach)', cursor: 'pointer', fontSize: 12, flex: 1 }}
                      >
                        {action}
                      </button>
                    ))}
        ...
        {tourActive && (
          <div style={{
            position: 'fixed', inset: 0, background: 'rgba(62,39,35,0.6)', zIndex: 180,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
