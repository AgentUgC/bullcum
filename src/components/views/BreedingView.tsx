  ...
  return (
    <div style={{ padding: 20, maxWidth: 1000, margin: '0 auto', height: '100%', overflow: 'auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 10, marginBottom: 20 }}>
        {tanks.map(tank => (
          <div key={tank.id} className="pixel-border-thin" style={{
            padding: 10, background: tank.occupied ? 'rgba(127,216,210,0.1)' : 'var(--warm-milk)',
            textAlign: 'center', position: 'relative',
          }}>
            <div style={{
              width: 40, height: 60, margin: '0 auto 6px',
              border: '2px solid var(--espresso)',
              borderRadius: '4px 4px 12px 12px',
              background: tank.occupied ? 'rgba(127,216,210,0.3)' : 'var(--cream)',
              position: 'relative', overflow: 'hidden',
            }}>
              {tank.occupied && (
                <div style={{
                  position: 'absolute', bottom: 0, left: 0, right: 0,
                  height: `${tank.progress}%`,
                  background: 'linear-gradient(180deg, var(--sci-cyan), #4FA8A0)',
      ...
      <div className="pixel-border" style={{ padding: 16, background: 'var(--cream)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
          <div>
            ...
            <select
              value={parentA || ''}
              onChange={e => setParentA(e.target.value || null)}
              style={{ width: '100%', padding: '6px 8px', border: '2px solid var(--espresso)', background: 'var(--cream)', fontSize: 12 }}
            >
          ...
          <div>
            ...
            <select
              value={parentB || ''}
              onChange={e => setParentB(e.target.value || null)}
              style={{ width: '100%', padding: '6px 8px', border: '2px solid var(--espresso)', background: 'var(--cream)', fontSize: 12 }}
            >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderTop: '1px dashed var(--light-wood)' }}>
          ...
          <button
            onClick={handleBreed}
            disabled={!canAfford || !canBreed}
            className="pixel-border"
            style={{
              padding: '8px 20px', background: canAfford && canBreed ? 'var(--sci-cyan)' : 'var(--light-wood)',
              color: canAfford && canBreed ? 'var(--espresso)' : 'var(--warm-brown)',
              cursor: canAfford && canBreed ? 'pointer' : 'not-allowed', fontSize: 13, fontWeight: 700,
            }}
          >
            启动培育
          </button>
