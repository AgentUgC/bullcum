          ...
          <div className="pixel-border" style={{ width: 220, padding: 12, background: 'var(--cream)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 300, overflow: 'auto' }}>
              {availableBulls.map(bull => (
                <button
                  key={bull.id}
                  onClick={() => { setSelectedBullId(bull.id); setGachaResult(null); }}
                  ...
                  style={{
                    ...
                    border: selectedBullId === bull.id ? '2px solid var(--espresso)' : '2px solid transparent',
            ...
            {selectedBull && (
              <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--light-wood)' }}>
                <div style={{ fontSize: 11, opacity: 0.7 }}>已选择: {selectedBull.name}</div>
              </div>
            )}
          </div>
          ...
          <div style={{ flex: 1, minWidth: 300 }}>
            <div className="pixel-border" style={{ padding: 20, background: 'var(--cream)', textAlign: 'center' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, marginBottom: 16 }}>
                {gachaResult ? gachaResult.map((card, i) => (
                  <div key={i} className="pixel-border" style={{
                    padding: 10, background: 'var(--warm-milk)', animation: 'slide-in-up 0.3s ease',
                  }}>
              ...
              <button
                onClick={drawGacha}
                disabled={gachaAnimating || !selectedBull}
                className="pixel-border"
                style={{
                  padding: '10px 28px',
                  background: gachaAnimating ? 'var(--light-wood)' : 'var(--rose-gold)',
                  color: '#fff',
                  cursor: gachaAnimating || !selectedBull ? 'not-allowed' : 'pointer',
                  fontFamily: 'var(--font-display)',
                  fontSize: 16,
                  animation: gachaAnimating ? 'shake 0.4s infinite' : 'none',
                }}
              >
              ...
              {gachaResult && (
                <div style={{ marginTop: 12, fontSize: 12, lineHeight: 1.6, color: 'var(--warm-brown)' }}>
                  场景预览: {selectedBull?.name} 以 {gachaResult[0].name} 被安置在大床上，本次采用 {gachaResult[1].name} 玩法。
                  种牛处于 {gachaResult[2].name} 状态。{gachaResult[3].name !== '无事发生' ? `突发: ${gachaResult[3].name}` : ''}
                </div>
              )}
            </div>
            ...
            <div className="pixel-border" style={{ marginTop: 12, padding: 12, background: 'var(--cream)' }}>
              ...
              <div style={{ width: '100%', height: 8, background: 'var(--light-wood)' }}>
                <div style={{
                  width: `${(state.manualUsed / state.manualLimit) * 100}%`,
                  height: '100%',
                  background: state.manualUsed > state.manualLimit * 0.8 ? 'var(--danger)' : 'var(--rose-gold)',
