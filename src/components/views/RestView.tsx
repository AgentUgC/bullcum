...
export default function RestView() {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div style={{ display: 'flex', gap: 6 }}>
            {(['all', 'standard', 'premium', 'superior'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)} className="pixel-border-thin" style={{
                padding: '4px 10px', fontSize: 11, cursor: 'pointer',
                background: filter === f ? 'var(--peach)' : 'var(--cream)',
              }}>
                ...
                <div key={zone} className="pixel-border" style={{ flex: 1, minWidth: 200, padding: 12, background: 'var(--cream)' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {zoneBulls.map(bull => (
                      <button
                        key={bull.id}
                        onClick={() => { setSelectedBullId(bull.id); setChatMessages([]); }}
                        className="pixel-border-thin"
                        style={{
                          width: 44, height: 44, borderRadius: '50%',
                          background: zone === 'superior' ? 'var(--q-superior)' : zone === 'premium' ? 'var(--q-premium)' : 'var(--q-standard)',
                          color: '#fff', fontSize: 10, fontWeight: 700,
                          cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
                          border: selectedBullId === bull.id ? '3px solid var(--espresso)' : '2px solid transparent',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 10 }}>
            {filteredBulls.map(bull => (
              <button
                ...
                style={{
                  padding: 10, background: 'var(--cream)', cursor: 'pointer', textAlign: 'left',
                  borderLeft: `4px solid ${bull.quality === 'superior' ? 'var(--q-superior)' : bull.quality === 'premium' ? 'var(--q-premium)' : 'var(--q-standard)'}`,
                }}
              >
      ...
      {/* Single Bull Interaction Room (Full Screen Overlay) */}
      {selectedBull && (
        <div style={{
          position: 'fixed', inset: 0, background: 'rgba(255,248,240,0.96)', zIndex: 200,
          display: 'flex', flexDirection: 'column', animation: 'fade-in 0.3s ease',
        }}>
          {/* Header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 20px', borderBottom: '2px solid var(--light-wood)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{
                ...
                background: selectedBull.quality === 'superior' ? 'var(--q-superior)' : selectedBull.quality === 'premium' ? 'var(--q-premium)' : 'var(--q-standard)',
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            ...
            <div style={{ width: '35%', padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRight: '1px solid var(--light-wood)' }}>
              <div className="pixel-border" style={{
                ...
                background: selectedBull.quality === 'superior' ? 'rgba(201,162,39,0.15)' : selectedBull.quality === 'premium' ? 'rgba(107,142,206,0.15)' : 'rgba(139,154,124,0.15)',
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: 16 }}>
                ...
                {chatMessages.map((msg, i) => (
                  <div key={i} style={{
                    ...
                    <div style={{
                      ...
                      fontSize: 12,
                      lineHeight: 1.6,
                      background: msg.sender === 'player' ? 'var(--peach)' : 'var(--warm-milk)',
                      border: `2px solid ${msg.sender === 'player' ? 'var(--rose-gold)' : 'var(--light-wood)'}`,
                      borderRadius: msg.sender === 'player' ? '12px 12px 0 12px' : '12px 12px 12px 0',
              ...
              <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                {['质检', '喂食', '接吻', '全身爱抚'].map(action => (
                  <button
                    ...
                    className="pixel-border"
                    style={{ padding: '6px 12px', background: 'var(--soft-pink)', cursor: 'pointer', fontSize: 12, flex: 1 }}
                  >
                    {action}
                  </button>
                ))}
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <input
                  ...
                  placeholder={`对 ${selectedBull.name} 说话...`}
                  style={{
                    flex: 1, padding: '8px 12px', border: '2px solid var(--espresso)', background: 'var(--cream)',
